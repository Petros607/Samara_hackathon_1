...
class Parser:
    ...
    def download_audio(self, url: str):
        ...
        if not os.path.exists(id):
            os.makedirs(id)
        if not os.path.exists(id + "/audio"):
            os.makedirs(id + "/audio")
    ...
    def get_metadata(self, url):
        ...
        teacher_name = soup.find('h5', id="room-owner-name",class_="font-weight-normal ml-4 mt-3 d-inline-block").get_text(strip=True).replace(" (Владелец)", "")
        ...
        for lection in lections:
            ...
            dict = { f'lection_{i}':{
                "id": None, "name_file": None, "name_teacher": teacher_name,"url": lection_url, "name_subject": lection_title,
    ...
    def download_image(self, url: str):
        ...
        if not os.path.exists(dir_name + "/slides"):
            os.makedirs(dir_name + "/slides")
        while True:
            try:
                save_path = os.path.join(dir_name + "/slides", "slide" + str(i) + ".svg")
